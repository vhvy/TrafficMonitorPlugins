...
{
    //TODO: 更改配置文件的文件名
    m_config_path = config_dir + L"Ping.ini";
    //TODO: 在此添加载入配置的代码

    utilities::CIniHelper ini(m_config_path);

	m_setting_data.gap_space = ini.GetBool(L"Setting", L"gap_space", false);
	m_setting_data.target_ip = ini.GetString(L"Setting", L"target_ip");
}

void CDataManager::SaveConfig() const
{
	bool gap_space = m_setting_data.gap_space;
	std::wstring target_ip = m_setting_data.target_ip;
    utilities::CIniHelper ini(m_config_path);
	ini.WriteBool(L"Setting", L"gap_space", gap_space);
	ini.WriteString(L"Setting", L"target_ip", target_ip);
	ini.Save();
}
